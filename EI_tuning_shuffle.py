...
# shuffle the keys
print(len(weights_allsubj.keys()))
keys_original = list(weights_allsubj.keys())
keys_shuffled = list(weights_allsubj.keys())

keys_ctrl_original = [k for k in weights_allsubj.keys() if k.startswith('ctrl')]
keys_ctrl_shuffled = [k for k in weights_allsubj.keys() if k.startswith('ctrl')]

np.random.seed(42)
np.random.shuffle(keys_shuffled)
np.save('keys_shuffled', keys_shuffled)
print(keys_original)
print(keys_shuffled)
...
for pat in range(32, len(keys_original)):
    
    weights = jnp.array(weights_allsubj[keys_original[pat]][83])
    lengths = jnp.array(len_allsubj[keys_original[pat]][83])
    ...
    target_fcdiag1 = jnp.array(FC_allsubj[keys_shuffled[pat]][83])
...
#   print(data_to_save)
    ...
    print(keys_original[pat], keys_shuffled[pat])
    filename = f"shuffle_numb_param_83_{keys_original[pat]}_{keys_shuffled[pat]}"
