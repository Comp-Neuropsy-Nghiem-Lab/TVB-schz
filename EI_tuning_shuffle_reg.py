...
# weights_allsubj = jnp.load('weights_allsubj_Hagmann.npy', allow_pickle=True).item()
weights_allsubj = jnp.load('/Users/lin/Documents/Bachelor thesis /SC_matrices/gFA_allsubj_Hagmann.npy', allow_pickle=True).item()

len_allsubj = jnp.load('len_allsubj_Hagmann.npy', allow_pickle=True).item()

old_reg_labels = list(range(83))
print (old_reg_labels)
new_reg_labels = list(range(83))
np.random.seed(42)
np.random.shuffle(new_reg_labels)
np.save('shuffled_reg_labels', new_reg_labels)
print (new_reg_labels)

# tupels for idx_old and idx_new 
# idx_old = list(itertools.product(old_reg_labels, repeat=2))
# print(range(len(old_reg_labels)))
# idx_new = list(itertools.product(new_reg_labels, repeat=2))

for pat in list(weights_allsubj.keys())[:]:
    print(pat)
    weights = jnp.array(weights_allsubj[pat][83])
    lengths = jnp.array(len_allsubj[pat][83])
    weights_np = np.array(weights)
    weights_shuffled = np.zeros_like(weights_np)
    n = weights_np.shape[0]
    # for i in range(n):
    #     for j in range(n):
    #         weights_shuffled[idx_new[i], idx_new[j]] = weights_np[i, j]
    weights_shuffled = weights[new_reg_labels, :][:, new_reg_labels]

    weights_shuffled = jnp.array(weights_shuffled)
    
    print (weights_shuffled)
    print(jnp.sum(weights))
    print(jnp.sum(weights_shuffled))

    region_labels = jnp.load('reg_labels_Hagmann83.npy')
    
    # print(keys_original[pat], keys_shuffled[pat])
    # Normalize weights to [0, 1] range
    weights = weights_shuffled / jnp.max(weights_shuffled)
    ...
    target_fcdiag1 = jnp.array(FC_allsubj[pat][83])
...
#   print(data_to_save)
    # print(len(keys_ctrl_original))
    # print(len(keys_ctrl_shuffled))
    filename = f"shuffle_reg_density_param_83_" + pat
